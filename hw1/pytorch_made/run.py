...
import sys
import os
module_par_dir = os.path.dirname(__file__)
sys.path.append(module_par_dir)
...
def run_epoch(split, upto=None):
    ...
    print("%s epoch average loss: %f" % (split, np.mean(np.array(lossfs))))
# ------------------------------------------------------------------------------
...
if __name__ == '__main__':
    ...
    model = MADE(xtr.shape[1], hidden_list, xtr.shape[1], num_masks=args.num_masks)
